    ...
    <form id="form-edit-fasilitas" action="{{ route('admin.fasilitas.update', $fasilitas->id_fasilitas) }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
            ...
            <select required name="gedung" id="gedung" class="cursor-pointer w-full border rounded-md px-3 py-2 text-sm">
                ...
                @foreach($gedung as $g)
                    <option value="{{ $g->id_gedung }}" {{ $fasilitas->ruangan->lantai->gedung->id_gedung == $g->id_gedung ? 'selected' : '' }}>{{ $g->nama_gedung }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select required name="kategori" id="kategori" class="cursor-pointer w-full border rounded-md px-3 py-2 text-sm">
                ...
                @foreach($kategori as $k)
                    <option value="{{ $k->id_kategori }}" {{ $fasilitas->id_kategori == $k->id_kategori ? 'selected' : '' }}>{{ $k->nama_kategori }}</option>
                @endforeach
            </select>
        ...
        <div class="col-span-2">
            ...
            <input type="text" name="nama_fasilitas" id="nama_fasilitas" class="w-full border rounded-md px-3 py-2 text-sm" value="{{ $fasilitas->nama_fasilitas }}" placeholder="Nama" required>
        ...
        <div>
            ...
            <select required name="urgensi" id="urgensi" class="cursor-pointer w-full border rounded-md px-3 py-2 text-sm">
                ...
                <option value="DARURAT" {{ $fasilitas->urgensi->value == 'DARURAT' ? 'selected' : '' }}>Darurat</option>
                <option value="PENTING" {{ $fasilitas->urgensi->value == 'PENTING' ? 'selected' : '' }}>Penting</option>
        ...
        <div class="col-span-2">
            <label class="block text-sm font-medium mb-1">Deskripsi Fasilitas <span class="text-red-500">*</span></label>
            <textarea name="deskripsi" id="deskripsi" rows="4" class="w-full border rounded-md px-3 py-2 text-sm resize-y" placeholder="Masukkan deskripsi fasilitas" required>{{ $fasilitas->deskripsi }}</textarea>
            <span id="deskripsi-error" class="text-xs text-red-500 mt-1 error-text"></span>
        </div>

        <div class="col-span-2">
            <label for="foto_fasilitas" class="block text-sm font-medium mb-1">Foto Fasilitas <span class="text-red-500">*</span></label>
            <div class="flex items-center border border-gray-300 rounded-md bg-white overflow-hidden">
                <input type="text" id="file-name-display" placeholder="{{ $fasilitas->foto_fasilitas ? $fasilitas->foto_fasilitas : 'Pilih File' }}" 
    ...
    if (gedungID) {
        $.get('/admin/fasilitas/get-lantai/' + gedungID, function (data) {
            ...
            $.each(data, function (key, val) {
                $('#lantai').append(`<option value="${val.id_lantai}" ${val.id_lantai == '{{ $fasilitas->ruangan->lantai->id_lantai }}' ? 'selected' : ''}>${val.nama_lantai}</option>`);
            });
            ...
            if (lantaiID) {
                $.get('/admin/fasilitas/get-ruangan/' + lantaiID, function (data) {
                    ...
                    $.each(data, function (key, val) {
                        $('#ruangan').append(`<option value="${val.id_ruangan}" ${val.id_ruangan == '{{ $fasilitas->id_ruangan }}' ? 'selected' : ''}>${val.nama_ruangan}</option>`);
                    });
