    ...
    <x-slot name="body">
        @forelse ($fasilitas as $index => $f)
            <x-table.row>
                ...
                <x-table.cell>
                    <span
                        class="px-2 py-1 rounded text-white w-full text-center {{ $f->kondisi->value === 'LAYAK' ? 'bg-green-500' : 'bg-red-500' }}">
                ...
                <x-table.cell>
                    ...
                    <button onclick="modalAction('{{ route('admin.fasilitas.edit_ajax', $f->id_fasilitas) }}')"
                        class="cursor-pointer text-blue-600 hover:underline text-sm ml-2">
