<x-table>
    <x-slot name="head">
        ...
        <x-table.heading>Ruangan</x-table.heading>
    ...
    <x-slot name="body">
        @forelse ($fasilitas as $index => $f)
            <x-table.row>
                ...
                <x-table.cell>{{ $f->ruangan->nama_ruangan ?? '-' }}</x-table.cell>
                <x-table.cell>
                    <span class="px-2 py-1 rounded text-white {{ $f->kondisi === \App\Http\Enums\Kondisi::BAIK ? 'bg-green-500' : 'bg-red-500' }}">
                        {{ $f->kondisi->value }}
                    </span>
                </x-table.cell>
                <x-table.cell>
                    <button onclick="modalAction('{{ route('admin.fasilitas.show_ajax', $f->id_fasilitas) }}')" class="text-blue-600 hover:underline text-sm">
                        <img src="{{ asset('icons/solid/Detail.svg') }}" alt="" class="h-7 w-7 inline">
                    </button>
                    <button onclick="modalAction('{{ route('admin.fasilitas.edit_ajax', $f->id_fasilitas) }}')" class="text-blue-600 hover:underline text-sm ml-2">
                        <img src="{{ asset('icons/solid/Edit.svg') }}" alt="" class="h-7 w-7 inline">
                    </button>
                    <button onclick="removeFasilitas('{{ $f->id_fasilitas }}')" class="text-red-600 hover:underline text-sm ml-2">
