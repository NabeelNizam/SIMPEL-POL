...
<div class="bg-white rounded-lg shadow-lg max-w-md w-full p-6 relative border-t border-blue-700">
    ...
    <h2 class="text-xl font-semibold mb-2 text-center">Tambah Data Jurusan</h2>
    <div class="w-[205px] h-1 bg-yellow-400 mx-auto mt-1 mb-6 rounded"></div>

    <form id="form-tambah-jurusan" action="{{ route('admin.jurusan.store') }}" method="POST" class="grid grid-cols-1 gap-4" enctype="multipart/form-data">
        @csrf

        <div class="mt-1">
            <label class="block text-sm font-medium mb-1">Kode Jurusan<span class="text-red-500">*</span></label>
            <input type="text" name="kode_jurusan" id="kode_jurusan" class="w-full border rounded-md px-3 py-2 text-sm" placeholder="Kode" required>
        ...
        <div>
            ...
            <input type="text" name="nama_jurusan" id="nama_jurusan" class="w-full border rounded-md px-3 py-2 text-sm" placeholder="Nama" required>
