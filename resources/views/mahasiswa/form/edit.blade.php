...
<div
    ...
    <form id="form-edit-aduan" action="{{ route('mahasiswa.form.update', $aduan->id_aduan) }}" method="POST"
            ...
            <select required name="gedung" id="gedung"
                ...
                @foreach ($gedung as $g)
                    <option value="{{ $g->id_gedung }}"
                        {{ $aduan->fasilitas->ruangan->lantai->gedung->id_gedung == $g->id_gedung ? 'selected' : '' }}>
        ...
        <div class="col-span-2">
            <label for="bukti_foto" class="block text-sm font-medium mb-1">Bukti Foto <span
            ...
            <div class="flex items-center border border-gray-300 rounded-md bg-white overflow-hidden">
                <input type="text" id="file-name-display"
                    placeholder="{{ $aduan->bukti_foto ? $aduan->bukti_foto : 'Pilih File' }}"
                    class="flex-grow px-3 py-2 text-sm {{ $aduan->bukti_foto ? 'text-black' : 'text-gray-500' }} bg-gray-50 border-none focus:ring-0 focus:outline-none"
                    readonly>
                ...
                <input type="file" id="bukti_foto" name="bukti_foto" accept=".jpg,.jpeg,.png"
                    class="hidden"
                    onchange="const input = document.getElementById('file-name-display'); input.value = this.files[0]?.name || '{{ $aduan->bukti_foto }}'; input.classList.remove('text-gray-500'); input.classList.add('text-black');">
            </div>
          ...
          <div class="col-span-2">
            <label class="block text-sm font-medium mb-1">Deskripsi<span class="text-red-500">*</span></label>
                <textarea name="deskripsi" id="deskripsi" rows="4" class="w-full border rounded-md px-3 py-2 text-sm resize-y" placeholder="Masukkan deskripsi fasilitas" required>{{ $aduan->deskripsi }}</textarea>
