...
<div
    class="bg-white rounded-lg shadow-lg max-w-3xl w-full p-6 relative space-y-6 border-t-4 border-blue-600 overflow-y-auto max-h-[80%]">
    {{-- Tombol Close --}}
    ...
    <form action="{{ route('teknisi.penugasan.update_ajax', $inspeksi->id_inspeksi) }}" method="POST">
        ...
        <div>
            ...
            <div class="flex flex-col lg:flex-row gap-6">
                <div class="lg:w-48">
                    <img src="{{ asset($fasilitas->foto_fasilitas ?? 'img/no-image.svg') }}" alt="Gambar Fasilitas"
                ...
                <div class="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                        ...
                        <span
                            class="inline-block mt-2 px-8 py-1 text-xs font-semibold text-white bg-red-600 rounded-lg">
                    ...
                    <div>
                        ...
                        <span
                            class="inline-block mt-2 px-8 py-1 text-xs font-semibold text-white bg-blue-600 rounded-lg">
                            {{ $statusAduan }}
                        </span>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-1 gap-4 text-sm text-gray-700">
                <div>
                    ...
                    <select name="tingkat_kerusakan" id="tingkat_kerusakan" class="w-full mt-2 p-2 border rounded">
                        <option value="PARAH" {{ $inspeksi->tingkat_kerusakan === 'PARAH' ? 'selected' : '' }}>Parah
                        </option>
                        <option value="SEDANG" {{ $inspeksi->tingkat_kerusakan === 'SEDANG' ? 'selected' : '' }}>Sedang
                        </option>
                        <option value="RINGAN" {{ $inspeksi->tingkat_kerusakan === 'RINGAN' ? 'selected' : '' }}>Ringan
                        </option>
                    </select>
                </div>
                <div>
        ...
        <div class="border border-dashed rounded-md p-4 mt-6">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm border border-gray-300 rounded-lg">
                    ...
                    <tbody id="anggaran-body">
                        ...
                        @empty
                            <tr id="empty-row">
                                <td colspan="4" class="border p-4 text-center text-gray-500">Tidak ada rincian anggaran.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                        ...
                        <tr>
                            <td colspan="4" class="border p-2 text-center">
                                <button type="button" onclick="addRow()"
                                    class="text-blue-600 hover:underline font-semibold cursor-pointer">+ Tambah
                                    Baris</button>
                            </td>
                        </tr>
                    </tfoot>
            ...
            <div class="text-right mt-4 font-semibold text-blue-600">
                Total: <span
                    id="total-anggaran">{{ number_format($inspeksi->biaya->sum('besaran'), 0, ',', '.') }}</span>
            </div>
        </div>
        ...
        <div class="text-right mt-6">
            <button type="submit"
                class="bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700 cursor-pointer">Simpan</button>
        </div>
    </form>
</div>
<script>
    function formatRupiah(number) {
        ...
        }).replace('Rp', 'Rp '); // Format untuk total tetap pakai "Rp"
    }

    function formatInputRupiah(input) {
        let value = input.value.replace(/[^,\d]/g, ''); // Hapus semua kecuali angka dan koma
        let number = parseInt(value) || 0;
        input.value = number.toLocaleString('id-ID'); // Format hanya angka tanpa "Rp"
        hitungTotalAnggaran(); // Perbarui total
    }
    ...
    function hitungTotalAnggaran() {
        ...
        document.getElementById('total-anggaran').textContent = formatRupiah(total); // Total tetap pakai "Rp"
    }

    function addRow() {
        const tbody = document.getElementById('anggaran-body');
        const index = tbody.querySelectorAll('tr:not(#empty-row)').length; // Hitung baris yang bukan empty-row
    ...
    document.addEventListener('DOMContentLoaded', () => {
        document.querySelectorAll('input[name*="[besaran]"]').forEach(input => {
            ...
            input.value = parseInt(value).toLocaleString('id-ID') || ''; // Format sebagai angka dengan pemisah ribuan
        });
        hitungTotalAnggaran();
    });
</script>
