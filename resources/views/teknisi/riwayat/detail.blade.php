<div class="bg-white rounded-lg shadow-lg max-w-3xl w-full p-6 relative space-y-6 border-t border-blue-600">
    ...
    <div class="lg:w-48">
        <img src="{{ asset($aduan->fasilitas->gambar ?? 'img/no-image.svg') }}"
    ...
    <div class="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
        ...
        <div>
            ...
            <p class="mt-2 font-semibold">{{ $perbaikan->tanggal_mulai ? \Carbon\Carbon::parse($perbaikan->tanggal_mulai)->format('d-m-Y') : '-' }}</p>
        </div>
        <div>
        ...
        <div>
            ...
            <p class="mt-2 font-semibold">{{ $perbaikan->tanggal_selesai ? \Carbon\Carbon::parse($perbaikan->tanggal_selesai)->format('d-m-Y') : '-' }}</p>
        </div>
